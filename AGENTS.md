...
- **后端** `server`（`@ns/server`）— Hono + Drizzle，已具备校园站 API 路由与测试
...
`src/index.ts` 为 Hono 入口；`src/routes/` 提供 spaces / articles / posts / feedbacks / feed / search / auth / me / notifications / favorites / reports / ai 等路由；`tests/api.test.ts` 覆盖主要 API 行为。
