    ...
    <header className={`sticky top-0 z-50 backdrop-blur-md border-b transition-colors duration-300 ${
               ...
               <>
                 ...
                 <div className="absolute top-full left-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 transition-all duration-200 transform origin-top-left z-20 animate-in fade-in zoom-in-95">
                    ...
                    {(Object.keys(domainMap) as Domain[]).map(domain => (
                      <button
                        key={domain}
                        onClick={() => { setDomain(domain); setDomainMenuOpen(false); navigate({ type: 'home' }); }}
            ...
            {isLoggedIn ? (
              <div className="relative group hidden md:block">
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50 py-2">
                   ...
                   <button onClick={() => navigate({ type: 'user-center', tab: 'profile' })} className="w-full text-left px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 flex items-center gap-3">
                     <User size={16} /> 个人中心
                   </button>
                   <button onClick={() => navigate({ type: 'admin' })} className="w-full text-left px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 flex items-center gap-3">
                     <Shield size={16} /> 后台管理
                   </button>
                   ...
                   <div className="border-t border-slate-50 mt-1 pt-1">
                     <button onClick={() => { setIsLoggedIn(false); navigate({ type: 'home' }); }} className="w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 flex items-center gap-3">
                       <LogOut size={16} /> 退出登录
                     </button>
                   </div>
                </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white/95 backdrop-blur-md p-4 space-y-4 animate-in slide-in-from-top-2 h-[calc(100vh-64px)] overflow-y-auto">
           {isLoggedIn && (
             <div className="flex items-center gap-3 px-4 py-4 bg-slate-50 rounded-xl mb-2" onClick={() => { navigate({ type: 'user-center', tab: 'profile' }); setMobileMenuOpen(false); }}>
           ...
           {!isLoggedIn ? (
             <button 
                onClick={() => { navigate({ type: 'login' }); setMobileMenuOpen(false); }}
              ...
              <div className="border-t border-slate-100 pt-4 space-y-1 mt-2">
                 <button onClick={() => { navigate({ type: 'user-center', tab: 'profile' }); setMobileMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-600 bg-slate-50 rounded-xl flex items-center gap-3">
                    <User size={18}/> 个人中心
                 </button>
                 <button onClick={() => { navigate({ type: 'admin' }); setMobileMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-600 bg-slate-50 rounded-xl flex items-center gap-3">
                    <Shield size={18} /> 后台管理
                 </button>
                 <button onClick={() => { setIsLoggedIn(false); navigate({ type: 'home' }); setMobileMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-red-500 bg-red-50 rounded-xl flex items-center gap-3">
                    <LogOut size={18}/> 退出登录
                 </button>
              </div>
           )}
